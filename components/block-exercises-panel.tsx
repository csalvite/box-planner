...
export function BlockExercisesPanel({
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(360px,0.95fr)] xl:items-start">
      <div className="space-y-4 rounded-lg border border-border/80 bg-background/45 p-4 shadow-sm sm:p-5">
        ...
        {!libraryQuery.isLoading && !libraryQuery.error ? (
          <div className="max-h-[48dvh] space-y-3 overflow-y-auto pr-1 md:max-h-none md:overflow-visible md:pr-0">
            {(libraryQuery.data ?? []).slice(0, 6).map((exercise) => (
              <div
                key={exercise.id}
                className="flex flex-col gap-3 rounded-lg border border-border/80 bg-card/60 p-3 shadow-sm sm:flex-row sm:items-start sm:justify-between"
              >
            ...
            {exercises.map((exercise) => (
              ...
              >
                ...
                <Button
                  ...
                  onClick={() => {
                    const deletePromise = deleteExercise.mutateAsync(
                      exercise.id,
                    );
